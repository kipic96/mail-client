...
namespace PublicTransportSystem
{
    class TimetableDatabase
    {
        ...
        {
            // TODO: Stworzyc caly koncept bazy danych na serwerze przez sql server, zaprojektowac w 
            ...
            using (SqlConnection conn = new SqlConnection())
            {
                ...
                conn.ConnectionString = @"Server=tcp:server-chilicki.database.windows.net,1433;Initial Catalog=Timetable;Persist Security Info=False;User ID=admin-chilicki;Password=P@$$w0|\d;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;";
                conn.Open();
                // Create the command
                SqlCommand command = new SqlCommand("tu nic nie ma na razie :d ", conn);
                // Add the parameters.
                command.Parameters.Add(new SqlParameter("value", 1));
